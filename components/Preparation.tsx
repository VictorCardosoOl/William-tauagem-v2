          ...
          {ITENS_PREPARO.map((item) => (
            <div key={item.id} className="group relative">
              ...
              <div className="mb-4 transform group-hover:-translate-y-2 transition-transform duration-500">
                <span className="font-serif italic font-thin text-8xl md:text-9xl text-ink-light opacity-60 group-hover:text-ink-medium group-hover:opacity-100 transition-all duration-500">
                  0{item.id}
                </span>
              </div>

              <h3 className="font-serif text-3xl font-light italic text-ink-black dark:text-white mb-6 relative inline-block">
                {item.titulo}
              </h3>

              <p className="font-sans text-xs leading-loose tracking-wide text-ink-dark dark:text-gray-400 font-light">
                {item.descricao}
              </p>
